...
class ApiRagRequestModel(BaseModel):
    ...
    sonchunk_setting: Dict = Field(..., description='子段分切设置')
...
@app.post(
    path='/Voicecomm/VoiceSageX/Rag/AddChunkAdvancedParent', 
        ...
        async with db.conn() as conn:
            async with conn.transaction():
                ...
                if record['preview_chunks']:
                    parent_document.metadata['idx'] = record['preview_chunks'][-1]["id"] + 1
                    logger.debug(f"{task_id} new parent chunk index: {parent_document.metadata['idx']}.")

                # 增加记录至表knowledge_base_doc_vector，并更新metadata
                for idx, chunk_record in enumerate(chunk_records):
                    # 生成子段序号，并在metadata中添加父段序号
                    s_idx = idx + 1
                    parent_document.children[idx].metadata['idx'] = s_idx
                    parent_document.children[idx].metadata['f_idx'] = parent_document.metadata['idx']
                    ...
                    # 更新切分策略
                    chunk_record[-2] = record['chunking_strategy']
                    ...
                    logger.debug(f'{task_id} inserted chunk-{idx} record: {id["id"]}.')
