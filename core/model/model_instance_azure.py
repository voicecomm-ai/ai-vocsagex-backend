...
@register_model_provider('azure')
class ModelInstanceAzure(ModelInstance):
    ...
    def to_Embeddings(self, **kwargs) -> Embeddings:
        if self.model_type == ModelInstanceType.Embedding:
            f_kwargs = filter_valid_args(AzureOpenAIEmbeddings, kwargs)
            if "model" in f_kwargs:
                f_kwargs.pop("model")
            if "api_key" in f_kwargs:
                f_kwargs.pop("api_key")
            if "azure_endpoint" in f_kwargs:
                f_kwargs.pop("azure_endpoint")
    ...
    def to_BaseChatModel(self, **kwargs) -> BaseChatModel:
        if self.model_type == ModelInstanceType.LLM:
            ...
            if "num_ctx" in temp_kwargs:
                temp_kwargs["max_completion_tokens"] = temp_kwargs.pop("num_ctx")
            if "num_predict" in temp_kwargs:
                temp_kwargs["max_completion_tokens"] = temp_kwargs.pop("num_predict")
            if "max_completion_tokens" not in temp_kwargs:
                temp_kwargs["max_completion_tokens"] = self.model_config.context_length
            if "presence_penalty" not in temp_kwargs:
                temp_kwargs["presence_penalty"] = 0.0
            if "api_version" not in temp_kwargs:
                temp_kwargs["api_version"] = "2023-07-01-preview"

            f_kwargs = filter_valid_args(AzureChatOpenAI, temp_kwargs)
            if "model" in f_kwargs:
                f_kwargs.pop("model")
            if "azure_endpoint" in f_kwargs:
                f_kwargs.pop("azure_endpoint")
